...
namespace eris {
    ...
    UnaryExprNode::UnaryExprNode(TokenType op, std::shared_ptr<Node> node, int line)
        : op(op), node(node) {
        this->line = line;
    }
    ...
    BinaryExprNode::BinaryExprNode(TokenType op, std::shared_ptr<Node> node_a, std::shared_ptr<Node> node_b, int line)
